...
if ($#ARGV < 2 || $#ARGV > 3) { # $#ARGV is the number of command line arguments minus 1
...
$ends =".ENDS";
$global = ".GLOBAL";
$outInSeparator = ":"; #This is ":" for multiple output ccc, but for backward compatibility,
		                  # we have to have nothing.

system("cp $glbparfile $outfile");

#open (IN,"<$infile") ||  die  ("Can't open $datafile : $!\n");
		...
		for($j=2;$j<=$#tokens;$j++)
		{
			if($tokens[$j] =~ /^([^\s]+)=([^\s]+)$/)
			{
			...
			else
		     {
					...
					elsif($tokens[$j] =~ /^[a-h]/) # detect the number of outputs to put the delimiter.
					{
							 $i ++;
					}
			}
...
#flag to indicate that we are at the beginnig of the mainlist 
$notTopCell = 0;
#flag to indicate we are inside the mainlist
$inTopCell = 0;
#%capnames = ();
while(<IN>)
{
...
# first check that if the Width is parameterized then its in parenthesis!
	...
	if ($notTopCell == 1 && /^[RrCcXxVvLl]/) #beginning of anything that belongs to the topCell
	{
		print OUT "$connect\n";
		$notTopCell = 0;
		$inTopCell = 1;
