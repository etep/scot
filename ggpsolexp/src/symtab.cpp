...
{
  if( currentIndex == vecString.size() ) return;
...
int symbol_table::makeAndGetIndexOfAuxLabel()
{
  ...
  std::string aux_name
  = AUX_LABEL_NAME_PREFIX + cnvt::intToString(numAuxLabels);
...
const std::string symbol_table::getSymbol
( const std::string & nodename, int ind,
...
unsigned symbol_table::getMaxSymbolLength() const
{
  ...
  for( int i = 0; i < vecString.size(); i ++ )
    if( vecString[i].length() > ml ) ml = vecString[i].length();
