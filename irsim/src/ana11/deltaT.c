...
private TimeType FindPreviousEdge( Trptr t, TimeType tm, hptr * edgeH ) {
   register hptr      h;
   register int       val;
...
private TimeType FindNextEdge( Trptr t, TimeType tm, hptr * edgeH ) {
   register hptr      h, p;
   register int       val;
...
private void        SetEdge2( XKeyEvent * ev );


private void Terminate( int cancel ) {
   if( cancel )
...
private void SetEdge1( XButtonEvent * ev ) {
   TimeType  tm;
   hptr      h;
...
public void DeltaT( char * s ) {
   PRINT( "\nt1 = " );
   
   Func x = (void (*)( void * )) (SetEdge1);
   SendEventTo( x );
