...
public void GetWindowGeometry( Window w, Coord * x, Coord * y, unsigned int * width, unsigned int * height, unsigned int * border ) {
   Window  root, subw;
   unsigned int     d;
...
public void GrabMouse( Window w, unsigned long ev_mask, Cursor cursor ) {
   while( XGrabPointer( display, w, False, ev_mask, GrabModeAsync,
                        GrabModeAsync, None, cursor, CurrentTime ) != GrabSuccess );
}


private int GetMotionEvent( Window w, XEvent * ev, Coord * x, Coord * y ) {
   unsigned int n;
   unsigned int mask;
   Coord         rx, ry;
   Window        root, subw;
...
private void MoveOutline( Window wind, Coord x1, Coord y1, Coord x2, Coord y2 ) {
...
public void MoveMe( Coord x, Coord y ) {
   Coord       dx, dy;
   Window      root;
   Coord       top, left;
   unsigned int w, h;
