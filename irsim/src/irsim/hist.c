...
public void SetFirstHist( nptr node, int value, int inp, long time ) {
    node->head.time = time;
    node->head.val = value;
    node->head.inp = inp;
 ...
 */
public void AddHist( nptr node, int value, int inp, long time, long delay, long rtime ) {
 ...
 */
public void AddPunted( nptr node, evptr ev, long tim ) {
 ...
 */
public void FreeHistList( nptr node ) {
 ...
 */
public void NewModel( int nmodel ) {
 ...
 */
public void NewEdge( nptr nd, evptr ev ) {
 ...
 */
public void DeleteNextEdge( nptr nd ) {
    register hptr  a, b, c;
...
public void FlushHist( Ulong ftime ) {
    register nptr  n;
...
public int backToTime( nptr nd, char * unused ) {
    register hptr  h, p;
    ...
    for( p = h, h = p->next; ; p = h, h = h->next ) {
        ...
        if( h->punt ) {
            ...
            if( qtime < cur_delta ) {
                ...
                enqueue_event( nd, ( int ) h->val, ( long ) h->t.p.delay, ( long ) h->t.p.rtime );
                cur_delta = tmp;
            }
            p->next = h->next;
            h->next = freeHist;
