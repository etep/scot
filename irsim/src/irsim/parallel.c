 ...
 * nodes on the list.
 */
public void make_parallel( nptr nlist ) {
    register nptr   n;
    ...
    for( cl = ~cl; nlist != NULL; nlist->nflags &= cl, nlist = nlist->n.next ) {
        for( l1 = nlist->nterm; l1 != NULL; l1 = l1->next ) {
            ...
            if ( type == SUBCKT ) { /* dont merge subckts */
                continue;
            }         /* we will loose the events if we
                  merge them */
#endif
            ...
            for( l2 = l1->next; l2 != NULL; prev = l2, l2 = l2->next ) {
                ...
                if( t2->ttype & ORED ) {
                    ...
                    Ffree( ( Object * )t2->r, sizeof( TranResist ) );
                    FREE_TRANS( t2 );
                }
                else {
...
public void UnParallelTrans( tptr t ) {
    tptr    tor;
    double  dr;
    ...
    if( tor->tlink == NULL ) {
        ...
        Ffree( ( Object * )tor->r, sizeof( TranResist ) );
        FREE_TRANS( tor );
    }
    else {
