...
#define Size_File_Head  ( NB_HEADER + NB_NUMBER + NB_NUMBER + NB_TIME + NB_NUMBER + NB_VERSION + NB_TIME )
...
private int DumpNodeHist( nptr nd, Ulong ndindx, char * cfp ) {
    FILE * fp = ( FILE * ) cfp;
    register hptr  h;
    Node_Head      header;
    File_Hist      hist;
    ...
    if( Fwrite( ( char * )&header, Size_Node_Head, fp ) <= 0 ) {
        goto abort;
    }
    ...
    for( h = h->next; h != last_hist; h = h->next ) {
        ...
        if( h->punt ) {
            ...
            if( Fwrite( ( char * )&hist, Size_PuntFile_Hist, fp ) <= 0 ) {
                goto abort;
            }
        }
        else {
            ...
            if( Fwrite( ( char * )&hist, Size_File_Hist, fp ) <= 0 ) {
                goto abort;
            }
        }
    }
    ...
    if( nd->events != NULL ) {
        ...
        if( Fwrite( ( char * )&h_end, Size_File_Hist, fp ) <= 0 ) {
            goto abort;
        }
        for( ev = nd->events; ev != NULL; ev = ev->nlink ) {
            ...
            if( Fwrite( ( char * )&pending, Size_File_Pend, fp ) <= 0 ) {
                goto abort;
            }
        }
    }
...
private int WriteHistHeader( FILE * fd ) {
    File_Head  fh;
    long       mag;
    ...
    if( Fwrite( ( char * )&fh, Size_File_Head, fd ) <= 0 ) {
        return( -1 );
    }
    return( 0 );
}
...
private int ReadHistHead( FILE * fd, long * pNewTime, long * pTime0 ) {
    File_Head  fh;
    int        n_nodes, hsize, n_version;
    long       newTime, magic, time0;

    if( Fread( ( char * )&fh, Size_File_Head, fd ) != Size_File_Head ) {
...
void walk_net_index( int ( *fun )( Node *, Ulong, char * ), char * arg );

public void DumpHist( char * fname ) {
    FILE * fp;
    ...
    walk_net_index( DumpNodeHist, ( char * )fp );

    ( void ) fclose( fp );
}


private int rd_hist( FILE * fd, nptr * pnlist ) {
    Node_Head     head;
    File_Hist     hist;
    EndHist    *   pe;
    ...
    pe = ( EndHist * ) &hist;
    ndlist = NULL;
    while( Fread( ( char * )&head, Size_Node_Head, fd ) == Size_Node_Head ) {
        ...
        while( TRUE ) {
            if( Fread( ( char * )&hist, Size_File_Hist, fd ) != Size_File_Hist ) {
                goto badfile;
            }
        ...
        while( n != 0 ) {
            ...
            if( Fread( ( char * )&pend, Size_File_Pend, fd ) != Size_File_Pend ) {
                goto badfile;
            }
...
private void fix_transistors( nptr nd ) {
    register lptr  l;
...
public void ReadHist( const char * fname ) {
    FILE    *   fd;
    long       newTime, time0;
    nptr       ndlist;
