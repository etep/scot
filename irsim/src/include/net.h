...
typedef struct	_aw {
	nptr	node; /* which node we will check */
...
#ifdef USER_SUBCKT
typedef struct _Subckt {
    ufun     model;                  /* model function for C sub-circuits */
    nptr     *nodes;                 /* internal state for C sub-circuit */
    uptr     udata;                  /* pointer to user-only data */
    short    nins;
    short    nouts;
    lptr     ndiode;
    char     *sname;
...
#ifdef  USER_SUBCKT
    Subckt   *subptr;                /* point to subcircuit structure */
#endif
  };
...
#define PSEC_TIME
#ifdef PSEC_TIME
#define	d2ns( D )		( (D) * 0.01 )		/* deltas to ns */
#define	d2ps( D )		( (D) * 10.0 )		/* deltas to ps */
#define	ns2d( N )		( (N) * 100.0 )		/* ns to deltas */
#define	ps2d( P )		( (P) * 0.1 )		/* ps to deltas */
#else
