/*************************** TESTNCHY.CPP ******************* AgF 2002-10-20 *
*                                                                            *
*  Test noncentral hypergeometric distribution                               *
...
void main() {
    ...
    long int delta;                      // step size in list
    ...
    NonCentralHypergeometricC nchyp( n, m, N, omega );

    // calculate approximate mean
    xme1 = nchyp.mean();

    // calculate exact mean and variance
    nchyp.moments( &xme2, &xva );
    ...
    // start message
    printf( "taking %li samples from noncentral hypergeometric distribution...", nn );

    // sample nn times
    for ( i=0; i<nn; i++ ) {

        // generate random number with desired distribution
        x = sto.NonCentralHypergeometric( n, m, N, omega );
