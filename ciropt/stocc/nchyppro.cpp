/************************** NCHYPPRO.CPP ********************** 2002-10-20 AF *
*
* Calculation of univariate and multivariate noncentral hypergeometric
* probability distribution.
*
* This file contains source code for the class NonCentralHypergeometricC
* and MultiNonCentralHypergeometricMoments defined in stocc.h.
*
* Documentation:
* ==============
...
#include "stocc.hpp"   // class definition
...
/***********************************************************************
             Methods for class NonCentralHypergeometricC
***********************************************************************/

NonCentralHypergeometricC::NonCentralHypergeometricC( long int n_, long int m_, long int N_, double odds, double accuracy_ ) {
    // constructor
    n = n_;
    m = m_;
    N = N_;
    omega = odds;
    accuracy = accuracy_; // set parameters
    xmin = m + n - N;
    if ( xmin < 0 ) { xmin = 0; } // calculate xmin and xmax
    xmax = n;
    if ( xmax > m ) { xmax = m; }
    ParametersChanged = 1;
    r = 1.;
...
double NonCentralHypergeometricC::mean( void ) {
    ...
    // calculate Cornfield mean of extended hypergeometric distribution as first guess
    a = ( m+n )*omega + ( N-m-n );
    ...
    if ( omega > 1. ) {
        do { // Newton Raphson iteration
            ...
            if ( mean > xmax ) { mean = xmax; }
            if ( ++iter > 40 ) {
                FatalError( "Search for mean failed" );
            }
        }
...
double NonCentralHypergeometricC::moments( double * mean_, double * var_ ) {
...
double NonCentralHypergeometricC::lnbico() {
...
void NonCentralHypergeometricC::findpars() {
    // calculate d, E, r, w
    ...
    do {
        ...
        if ( zd == 0 ) { FatalError( "can't find r" ); }
        rr -= z / zd;
        if ( rr <= d1 ) { rr = lastr * 0.125 + d1*0.875; }
        if ( ++j == 70 ) { FatalError( "convergence problem searching for r" ); }
    }
    ...
    if ( phi2d > 0. ) { FatalError( "peak width undefined in function findpars" ); }
    w = 1./sqrt( -phi2d );
}


/***********************************************************************
         calculation methods in class NonCentralHypergeometricC
***********************************************************************/

double NonCentralHypergeometricC::recursive() {
    // recursive calculation
    // noncentral hypergeometric distribution by recursion formula
    ...
    p1[-1] = 0.;
    p1[0]  = 1.;          // initialize for recursion
    x1 = x2 = 0;
    for ( nu=1; nu<=n; nu++ ) {
        if ( n - nu < x - x1 || p1[x1] < accuracya ) {
            x1++;               // increase lower limit when breakpoint passed or probability negligible
            p2--;
        }              // compensate buffer offset in order to reduce storage space
        if ( x2 < x && p1[x2] >= accuracya ) {
            x2++;
            y1 = 0.;
        }    // increase upper limit until x has been reached
        else {
            y1 = p1[x2];
        }
        if ( x1 > x2 ) { return 0.; }
        if ( p2+x2-p > BUFSIZE ) { FatalError( "buffer overrun in function recursive" ); }
...
double NonCentralHypergeometricC::binoexpand() {
    ...
    FatalError( "x > 1 not supported by function binoexpand" );
    return 0;
}


double NonCentralHypergeometricC::laplace() {
    ...
    if ( accuracy < 1.E-10 && epsilonmax >= 8. ) {
        epsilon = 8.;
        erfres = erfres80;
        erfreslen = sizeof( erfres80 )/sizeof( erfres80[0] );
    }
    else if ( accuracy < 1.E-8 && epsilonmax >= 7. ) {
        epsilon = 7.;
        erfres = erfres70;
        erfreslen = sizeof( erfres70 )/sizeof( erfres70[0] );
    }
    else if ( accuracy < 1.E-6 && epsilonmax >= 6. ) {
        epsilon = 6.;
        erfres = erfres60;
        erfreslen = sizeof( erfres60 )/sizeof( erfres60[0] );
    }
    else {
        epsilon = 5.;
        erfres = erfres50;
        erfreslen = sizeof( erfres50 )/sizeof( erfres50[0] );
    }
    if ( epsilon > epsilonmax ) { FatalError( "Laplace failed. peak width too high" ); }
    degree = MAXDEG;
...
double NonCentralHypergeometricC::integrate() {
    ...
    if ( w < 0.02 || w < 0.1 && ( x==m || n-x==N-m ) && accuracy > 1E-6 ) {
        ...
        for ( t1=0., t2=0.5; t1 < 1.; t1+=0.5, t2+=0.5 ) {
            ...
            delta = tinf - t1;
            if ( delta > t2 - tinf ) { delta = t2 - tinf; } // distance to nearest endpoint
...
double NonCentralHypergeometricC::integrate_step( double ta, double tb ) {
...
double NonCentralHypergeometricC::search_inflect( double t_from, double t_to ) {
    ...
    do {
        ...
        if ( ++iter > 20 ) { FatalError( "Search for inflection point failed in function search_inflect" ); }
    }
...
double NonCentralHypergeometricC::probability( long int x_ ) {
...
int NonCentralHypergeometricC::MakeTable( double * table, long int MaxLength, long int * start, long int * end ) {
    // Makes a table of the noncentral hypergeometric probability distribution.
    ...
    if ( area < 4000 || area < 10000 && N > 1000.*n ) {
        ...
        for ( nu=1; nu<=n; nu++ ) {
            ...
            if ( p2 + x2 - p > BUFSIZE || x1 > x2 ) {
                goto ONE_BY_ONE;
            }              // Error: table length exceeded. Use other method
        ...
        *end = x1 + i2 - 1;
        ...
        return i1==i2;
    }                    // true if table size not reduced
...
/***********************************************************************
         calculation methods in class MultiNonCentralHypergeometricC
***********************************************************************/

MultiNonCentralHypergeometricC::MultiNonCentralHypergeometricC( long int n_, long int * m_, double * odds_, int colors_, float accuracy_ ) {
    // constructor
    long int N1;
    int i;
    n = n_;
    ...
    for ( N=N1=0, i=0; i < colors; i++ ) {
        if ( m[i] < 0 || omega[i] < 0 ) { FatalError( "Parameter negative in constructor for MultiNonCentralHypergeometricC" ); }
        N += m[i];
        if ( omega[i] ) { N1 += m[i]; }
    }
    if ( N < n ) { FatalError( "Not enough items in constructor for MultiNonCentralHypergeometricC" ); }
    if ( N1< n ) { FatalError( "Not enough items with nonzero weight in constructor for MultiNonCentralHypergeometricC" ); }
}


void MultiNonCentralHypergeometricC::mean( double * mu ) {
    // calculate approximate mean of multivariate noncentral hypergeometric
    ...
    do {
        ...
        if ( t >= 0 ) { t = 0.5 * t1; }
        if ( ++iter > 20 ) {
            FatalError( "Search for mean failed in MultiNonCentralHypergeometricC::mean" );
        }
    }
    while ( fabs( H - n ) > 1E-3 );
...
double MultiNonCentralHypergeometricC::binoexpand( void ) {
    ...
    }                      // find the nonzero x[i]
    if ( k > 1 ) { FatalError( "More than one x[i] nonzero in MultiNonCentralHypergeometricC::binoexpand" ); }
    return exp( FallingFactorial( m[j],n ) - FallingFactorial( W/omega[j],n ) );
}


double MultiNonCentralHypergeometricC::lnbico( void ) {
    // natural log of binomial coefficients
    bico = 0.;
    int i;
    for ( i=0; i<colors; i++ ) {
...
void MultiNonCentralHypergeometricC::findpars( void ) {
    // calculate r, w, E
    // calculate d, E, r, w
...
double MultiNonCentralHypergeometricC::laplace( void ) {
    // Laplace's method with narrow integration interval,
    // using error function residue table, defined in erfres.cpp
    // NOTE: This function can only be used when the integrand peak is narrow.
    // findpars() must be called before this function.
...
double MultiNonCentralHypergeometricC::integrate( void ) {
    // non-central hypergeometric distribution function
    // calculation by numerical integration with variable-length steps
    // NOTE: findpars() must be called before this function.
    double s;                            // result of integration step
...
double MultiNonCentralHypergeometricC::integrate_step( double ta, double tb ) {
    // integration subprocedure used by integrate()
    // makes one integration step from ta to tb using Gauss-Legendre method.
    // result is scaled by multiplication with exp(bico)
    double ab, delta, tau, ltau, y, sum, taur, rdm1;
...
#error // IPOINTS must be a value for which the tables are defined
    ...
    for ( j = 0; j < IPOINTS; j++ ) {
        ...
        for ( i = 0; i < colors; i++ ) {
            // possible loss of precision due to subtraction here:
            if ( omega[i] ) {
                y += pow1pow( taur*omega[i],x[i] );
            }
        } // ln((1-e^taur*omegai)^xi)
        y += rdm1*ltau + bico;
...
double MultiNonCentralHypergeometricC::search_inflect( double t_from, double t_to ) {
    // search for an inflection point of the integrand PHI(t) in the interval
    // t_from < t < t_to
    double t, t1;                        // independent variable
...
double MultiNonCentralHypergeometricC::probability( long int * x_ ) {
    // calculate probability function. choosing best method
    int i, j, em;
    long int xsum;
    x = x_;
    ...
    if ( xsum != n ) {
        FatalError( "sum of x values not equal to n in function MultiNonCentralHypergeometricC::probability" );
    }
    ...
    if ( colors < 3 ) {
        ...
        return NonCentralHypergeometricC( n,m[0],N,omega[0]/omega[1],accuracy ).probability( x[0] );
    }
...
/***********************************************************************
         Methods for MultiNonCentralHypergeometricMoments
***********************************************************************/

double MultiNonCentralHypergeometricMoments::moments( double * mu, double * variance, long int * combinations ) {
    // calculates mean and variance of the noncentral hypergeometric
    // distribution by calculating all combinations of x-values.
...
double MultiNonCentralHypergeometricMoments::loop( long int n, int c ) {
