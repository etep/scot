/************************** XHYPPRO.CPP *********************** 2002-10-20 AF *
*
* Calculation of univariate and multivariate extended hypergeometric
* probability distribution.
*
* This file contains source code for the class ExtendedHypergeometricC
* and MultiExtendedHypergeometricC defined in stocc.h.
*
* Documentation:
* ==============
...
/***********************************************************************
             Methods for class ExtendedHypergeometricC
***********************************************************************/

ExtendedHypergeometricC::ExtendedHypergeometricC( long int n_, long int m_, long int N_, double odds_ ) {
    // constructor
    // set parameters
    n = n_;
    m = m_;
    ...
    if ( n<0 || m<0 || N<0 || odds<0. || n>N || m>N ) { FatalError( "Parameter out of range in class ExtendedHypergeometricC" ); }
    // initialize
    logodds = log( odds );
    scale = rsum = 0.;
    ParametersChanged = 1;
...
double ExtendedHypergeometricC::mean( void ) {
...
double ExtendedHypergeometricC::moments( double * mean_, double * var_ ) {
...
double ExtendedHypergeometricC::probability( long int x ) {
    ...
    if ( !rsum ) {
        ...
        x2 = x1 + 1;
        scale = 0.;
        scale = lng( x1 );  // calculate scale to avoid overflow
        rsum = 1.;                      // = exp(lng(x1)) with this scale
        for ( x1--; x1 >= xmin; x1-- ) {
            rsum += y = exp( lng( x1 ) ); // sum from x1 and down
            if ( y < accuracy ) { break; }
        }     // until value becomes negligible
...
double ExtendedHypergeometricC::probabilityRatio( long int x, long int x0 ) {
    ...
    if ( x < xmin || x > xmax ) { return 0.; }
    if ( x0 < xmin || x0 > xmax ) { FatalError( "Infinity in ExtendedHypergeometricC::probabilityRatio" ); }
    if ( dx == 0. ) { return 1.; }
    if ( dx < 0. ) {
        invert = 1;
        dx = -dx;
...
double ExtendedHypergeometricC::lng( long int x ) {
...
/***********************************************************************
         calculation methods in class MultiExtendedHypergeometricC
***********************************************************************/

MultiExtendedHypergeometricC::MultiExtendedHypergeometricC( long int n_, long int * m_, double * odds_, int colors_, float accuracy_ ) {
    // constructor
    long int N1;
    int i;
    // copy parameters
    ...
    for ( N=N1=0, i=0; i < colors; i++ ) {
        if ( m[i] < 0 || odds[i] < 0 ) { FatalError( "Parameter negative in constructor for MultiExtendedHypergeometricC" ); }
        N += m[i];
        if ( odds[i] ) { N1 += m[i]; }
    }
    if ( N < n ) { FatalError( "Not enough items with in constructor for MultiExtendedHypergeometricC" ); }
    if ( N1< n ) { FatalError( "Not enough items with nonzero weight in constructor for MultiExtendedHypergeometricC" ); }
...
void MultiExtendedHypergeometricC::mean( double * mu ) {
    // calculates approximate mean of multivariate extended hypergeometric
    ...
    if ( colors < 3 ) {
        ...
        if ( colors == 1 ) { mu[0] = n; }
        if ( colors == 2 ) {
            mu[0] = ExtendedHypergeometricC( n,m[0],m[0]+m[1],odds[0]/odds[1] ).mean();
            mu[1] = n - mu[0];
        }
        return;
    }
    ...
    do {
        ...
        r *= n * ( N-q ) / ( q * ( N-n ) );
        if ( ++iter > 100 ) { FatalError( "convergence problem in function MultiExtendedHypergeometricC::mean" ); }
    }
...
void MultiExtendedHypergeometricC::variance( double * var ) {
    // calculates approximate variance of multivariate extended
...
double MultiExtendedHypergeometricC::probability( long int * x ) {
    // Calculate probability function.
...
double MultiExtendedHypergeometricC::moments( double * mean, double * variance, long int * combinations ) {
    // calculates mean and variance of the extended hypergeometric
...
void MultiExtendedHypergeometricC::SumOfAll() {
...
double MultiExtendedHypergeometricC::loop( long int n, int c ) {
...
double MultiExtendedHypergeometricC::lng( long int * x ) {
