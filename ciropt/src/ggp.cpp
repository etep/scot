...
bool ggp::toMatlabFile( const std::string & fileName ) {
   ...
   wrt << "A = zeros( "
       << ( objFunc.getNumMonomials() + sumNumMono ) << ", "
       << symtab.getNumberOfSymbols() << " );" << endl;
   wrt << "b = zeros( "
       << ( objFunc.getNumMonomials() + sumNumMono ) << ", 1);" << endl;
...
void ggp::solveUsingMOSEKdgopt( const std::string & fileName, ggp & GGP ) {
   ...
   //system( ("mpsToEo.pl " + mpsFileName + " " + fFileName + " " + eoFileName).c_str() );
   //system( ("mskexpopt " + eoFileName + " -primal" + " -p ~/research/dco/ggpsol/paramFile").c_str())
   const char * scotHomePtr    = getenv( "SCOT_HOME_DIR" );
   const std::string scotHome  = std::string( scotHomePtr );
   const std::string paramFile = scotHome + "/txt/paramFile";
   
   system( ( "mskdgopt " + mpsFileName + " " + fFileName + " " + paramFile ).c_str() );

   system( ( "rm " + mpsFileName + " " + fFileName ).c_str() );
   ...
   int status = dual2primal( "dgopt.sol", numVar, numCon, primal_solution, at_act, u_lower, &p_obj, &d_obj );
   ...
   if( status != -4 ) system( "rm dgopt.sol" );
